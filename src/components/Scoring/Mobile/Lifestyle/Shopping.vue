...
// const shoppingTransData = ref([])

// API Call: Get Shopping Transactions Data
const loadShoppingTransData = async () => {
  // await axiosInstance
  //   .get("/e_statement/")
  //   .then(response => (shoppingTransData.value = response.data))
  //   .catch(error => console.error(error));
};

onMounted(() => {
  loadShoppingTransData();
});
</script>
            ...
            <v-container fluid>
              <div class="mx-4">
                <h1 class="text-h6 font-weight-regular">
                  Shopping Outlets Transactions
                </h1>
                <h2 class="text-caption text-grey-darken-2 font-weight-regular">
                  Summary of Shopping Outlets Transactions
                </h2>
              </div>
              <div class="my-8 mx-4">
                ...
                <v-row class="justify-space-between d-flex">
                  ...
                  <v-col>{{ shopping.count.received }}</v-col>
                  <v-col>{{ shopping.count.paid }}</v-col>
                </v-row>
                <v-divider class="my-2" />
                <v-row class="justify-space-between d-flex">
                  <v-col class="font-weight-medium">Highest</v-col>
                  <v-col>{{ shopping.highest.received }}</v-col>
                  <v-col>{{ shopping.highest.paid }}</v-col>
                </v-row>
                <v-divider class="my-2" />
                <v-row class="justify-space-between d-flex">
                  <v-col class="font-weight-medium">Lowest</v-col>
                  <v-col>{{ shopping.lowest.received }}</v-col>
                  <v-col>{{ shopping.lowest.paid }}</v-col>
                </v-row>
                <v-divider class="my-2" />
                <v-row class="justify-space-between d-flex">
                  <v-col class="font-weight-medium">Last</v-col>
                  <v-col>{{ shopping.last.received }}</v-col>
                  <v-col>{{ shopping.last.paid }}</v-col>
                </v-row>
                <v-divider
                  class="my-3"
                  :thickness="3"
                ...
                <v-row class="font-weight-bold justify-space-between d-flex">
                  <v-col>Total</v-col>
                  <v-col>{{ shopping.total.received }}</v-col>
                  <v-col>{{ shopping.total.paid }}</v-col>
                </v-row>
              </div>
            </v-container>
          </v-card>
      ...
      <v-row>
        <v-container fluid>
          ...
          >
            <div class="px-8">
              <h1 class="text-h6 font-weight-regular">
                Top Shopping Outlets Transactions
              </h1>
              <h2 class="text-caption text-grey-darken-2 font-weight-regular">
                Summary of Shopping Outlets Transactions
              </h2>
