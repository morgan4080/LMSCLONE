...
const loadHotelsTransBuyGoodsData = async () => {
  await axiosInstance
    .get(
      `/e_statement/buy_goods_classifications_summary?statementRefId=${route.params.slug}&classification=Hotel&pageSize=100&sortBy=id`
    )
...
const loadHotelsTopTransData = async () => {
  await axiosInstance
    .get(
      `/e_statement/top_paybill_classifications?statementRefId=${route.params.slug}&classification=Hotel&pageSize=${itemsPerPage.value}&sortBy=id`
    )
