...
const loadBettingTransReceivedData = async () => {
  await axiosInstance
    .get(
      `/e_statement/pay_bill_classifications_received?idNumber=${route.params.slug}&classification=Betting&pageSize=100&sortBy=id`
    )
...
const loadBettingTransSentData = async () => {
  await axiosInstance
    .get(
      `/e_statement/pay_bill_classifications_sent?idNumber=${route.params.slug}&classification=Betting&pageSize=100&sortBy=id`
    )
...
const loadBettingTransBuyGoodsData = async () => {
  await axiosInstance
    .get(
      `/e_statement/buy_goods_classifications_summary?idNumber=${route.params.slug}&classification=Betting&pageSize=100&sortBy=id`
    )
...
const loadBettingTopTransData = async () => {
  await axiosInstance
    .get(
      `/e_statement/top_paybill_classifications?idNumber=${route.params.slug}&pageSize=${itemsPerPage.value}&sortBy=id`
    )
    .then(
      response =>
        (bettingTopTransData.value = response.data.content.filter(
          (item: BettingTopTransData) => item.classification === "Betting"
        ))
    )
    .catch(error => console.error(error));
};
            ...
            <v-container fluid>
              ...
              <div class="mx-4 my-8">
                ...
                <v-row class="justify-space-between d-flex">
                  ...
                  <v-col>{{
                    formatter(bettingTransReceivedData[0]?.highest)
                  }}</v-col>
                  <v-col>{{
                    formatter(bettingTransSentData[0]?.highest)
                  }}</v-col>
                </v-row>
                ...
                <v-row class="justify-space-between d-flex">
                  ...
                  <v-col>{{
                    formatter(bettingTransReceivedData[0]?.lowest)
                  }}</v-col>
                  <v-col>{{
                    formatter(bettingTransSentData[0]?.lowest)
                  }}</v-col>
                </v-row>
                <v-divider class="my-2" />
                <v-row class="justify-space-between d-flex">
                ...
                <v-row class="font-weight-bold justify-space-between d-flex">
                  <v-col>Total</v-col>
                  <v-col>{{
                    formatter(bettingTransReceivedData[0]?.total)
                  }}</v-col>
            ...
            <v-container fluid>
              ...
              <div class="mx-4 my-8">
                ...
                <v-row class="justify-space-between d-flex">
                  ...
                  <v-col> - </v-col>
                  <v-col>{{
                    formatter(bettingTransBuyGoodsData[0]?.highest)
                  }}</v-col>
                </v-row>
                <v-divider class="my-2" />
                <v-row class="justify-space-between d-flex">
                ...
                <v-row class="justify-space-between d-flex">
                  ...
                  <v-col> - </v-col>
                  <v-col>{{
                    formatter(bettingTransBuyGoodsData[0]?.lowest)
                  }}</v-col>
                </v-row>
                <v-divider class="my-2" />
                <v-row class="justify-space-between d-flex">
                ...
                <v-row class="font-weight-bold justify-space-between d-flex">
                  ...
                  <v-col> - </v-col>
                  <v-col>{{
                    formatter(bettingTransBuyGoodsData[0]?.total)
                  }}</v-col>
                </v-row>
              </div>
            </v-container>
