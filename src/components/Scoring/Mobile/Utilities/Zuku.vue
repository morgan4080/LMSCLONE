...
const loadZukuTransSentData = async () => {
  await axiosInstance
    .get(
      `/e_statement/pay_bill_classifications_sent?statementRefId=${route.params.slug}&classification=Zuku&pageSize=100&sortBy=id`
    )
...
const loadZukuTopTransData = async () => {
  await axiosInstance
    .get(
      `/e_statement/top_paybill_classifications?statementRefId=${route.params.slug}&classification=Zuku&pageSize=${itemsPerPage.value}&sortBy=id`
    )
            ...
            <v-container fluid>
              ...
              <div class="mx-4 my-8">
                ...
                <v-row class="justify-space-between d-flex">
                  ...
                  <v-col> - </v-col>
                  <v-col>{{ formatter(zukuTransSentData[0]?.highest) }}</v-col>
                </v-row>
                ...
                <v-row class="font-weight-bold justify-space-between d-flex">
                  <v-col>Total</v-col>
                  <v-col> - </v-col>
                  <v-col>{{ formatter(zukuTransSentData[0]?.total) }}</v-col>
                </v-row>
              </div>
            </v-container>
          </v-card>
