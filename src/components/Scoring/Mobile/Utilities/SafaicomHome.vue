...
const loadSafaricomTransSentData = async () => {
  await axiosInstance
    .get(
      `/e_statement/pay_bill_classifications_sent?statementRefId=${route.params.slug}&classification=SafaricomHome&pageSize=100&sortBy=id`
    )
...
const loadSafaricomTopTransData = async () => {
  await axiosInstance
    .get(
      `/e_statement/top_paybill_classifications?statementRefId=${route.params.slug}&classification=SafaricomHome&pageSize=${itemsPerPage.value}&sortBy=id`
    )
