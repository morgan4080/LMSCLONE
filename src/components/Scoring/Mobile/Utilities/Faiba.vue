...
const totalItems = computed(() => faibaTopTransData.value.length);
const headers = ref<
...
>([
  ...
  { title: "Faiba Name", key: "name", align: "start", sortable: false },
...
const faibaTransSentData = ref<FaibaDataItem[]>([]);
const faibaTopTransData = ref<FaibaTopTransData[]>([]);

const loadFaibaTransSentData = async () => {
  await axiosInstance
    .get(
      `/e_statement/pay_bill_classifications_sent?statementRefId=${route.params.slug}&classification=FaibaJTL&pageSize=100&sortBy=id`
    )
...
const loadFaibaTopTransData = async () => {
  await axiosInstance
    .get(
      `/e_statement/top_paybill_classifications?statementRefId=${route.params.slug}&classification=FaibaJTL&pageSize=${itemsPerPage.value}&sortBy=id`
    )
            ...
            <v-container fluid>
              ...
              <div class="mx-4 my-8">
                ...
                <v-row class="justify-space-between d-flex">
                  ...
                  <v-col> - </v-col>
                  <v-col>{{ formatter(faibaTransSentData[0]?.highest) }}</v-col>
                </v-row>
                <v-divider class="my-2" />
                <v-row class="justify-space-between d-flex">
                  <v-col class="font-weight-medium">Highest To</v-col>
                  <v-col> - </v-col>
                  <v-col>{{ faibaTransSentData[0]?.highest_who }}</v-col>
                </v-row>
                <v-divider class="my-2" />
                <v-row class="justify-space-between d-flex">
                  <v-col class="font-weight-medium">Lowest</v-col>
                  <v-col> - </v-col>
                  <v-col>{{ formatter(faibaTransSentData[0]?.lowest) }}</v-col>
                </v-row>
                <v-divider class="my-2" />
                <v-row class="justify-space-between d-flex">
                  <v-col class="font-weight-medium">Lowest To</v-col>
                  <v-col> - </v-col>
                  <v-col>{{ faibaTransSentData[0]?.lowest_who }}</v-col>
                </v-row>
                <v-divider
                  class="my-3"
                  :thickness="3"
                ...
                <v-row class="font-weight-bold justify-space-between d-flex">
                  <v-col>Total</v-col>
                  <v-col> - </v-col>
                  <v-col>{{ formatter(faibaTransSentData[0]?.total) }}</v-col>
                </v-row>
              </div>
            </v-container>
          </v-card>
        ...
        <v-container fluid>
          ...
          >
            ...
            <v-data-table-server
              ...
              :items="faibaTopTransData"
              :loading="loading"
              loading-text="Loading...Please Wait"
              item-value="name"
              @update:options="loadFaibaTopTransData()"
