...
// const waterTransData = ref([])

// API Call: Get Water Transactions Data
const loadWaterTransData = async () => {
  // await axiosInstance
  //   .get("/e_statement/")
  //   .then(response => (waterTransData.value = response.data))
...
<template>
  <v-container fluid>
    ...
    >
      <p>Water And Sewerage Services</p>
      <v-icon
            ...
            <v-container fluid>
              ...
              <div class="my-8 mx-4">
                ...
                <v-row class="justify-space-between d-flex">
                  ...
                  <v-col>{{ water.count.received }}</v-col>
                  <v-col>{{ water.count.paid }}</v-col>
                </v-row>
                <v-divider class="my-2" />
                <v-row class="justify-space-between d-flex">
                  <v-col class="font-weight-medium">Highest</v-col>
                  <v-col>{{ water.highest.received }}</v-col>
                  <v-col>{{ water.highest.paid }}</v-col>
                </v-row>
                <v-divider class="my-2" />
                <v-row class="justify-space-between d-flex">
                  <v-col class="font-weight-medium">Lowest</v-col>
                  <v-col>{{ water.lowest.received }}</v-col>
                  <v-col>{{ water.lowest.paid }}</v-col>
                </v-row>
                <v-divider class="my-2" />
                <v-row class="justify-space-between d-flex">
                  <v-col class="font-weight-medium">Last</v-col>
                  <v-col>{{ water.last.received }}</v-col>
                  <v-col>{{ water.last.paid }}</v-col>
                </v-row>
                <v-divider
                  class="my-3"
                  :thickness="3"
                ...
                <v-row class="font-weight-bold justify-space-between d-flex">
                  <v-col>Total</v-col>
                  <v-col>{{ water.total.received }}</v-col>
                  <v-col>{{ water.total.paid }}</v-col>
                </v-row>
              </div>
            </v-container>
          </v-card>
