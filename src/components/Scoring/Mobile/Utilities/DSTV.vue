...
const loadDstvTransSentData = async () => {
  await axiosInstance
    .get(
      `/e_statement/pay_bill_classifications_sent?statementRefId=${route.params.slug}&classification=DstvOrGotv&pageSize=100&sortBy=id`
    )
...
const loadDstvTopTransData = async () => {
  await axiosInstance
    .get(
      `/e_statement/top_paybill_classifications?statementRefId=${route.params.slug}&classification=DstvOrGotv&pageSize=${itemsPerPage.value}&sortBy=id`
    )
