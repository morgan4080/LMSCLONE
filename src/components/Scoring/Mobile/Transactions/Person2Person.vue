...
const loadRerson2personTransReceivedData = async () => {
  await axiosInstance
    .get(
      `/e_statement/top_customers_received?statementRefId=${route.params.slug}&pageSize=${itemsPerPage.value}&sortBy=id`
    )
    .then(response => {
...
const loadRerson2personTransSentData = async () => {
  await axiosInstance
    .get(
      `/e_statement/top_customers_sent?statementRefId=${route.params.slug}&pageSize=${itemsPerPage.value}&sortBy=id`
    )
    .then(
