...
const loadAgentTransData = async () => {
  await axiosInstance
    .get(
      `/e_statement/agent_transaction_summary?statementRefId=${route.params.slug}&pageSize=100&sortBy=id`
    )
...
const loadAgentTopTransData = async () => {
  await axiosInstance
    .get(
      `/e_statement/top_agent_transactions?statementRefId=${route.params.slug}&pageSize=${itemsPerPage.value}&sortBy=id`
    )
