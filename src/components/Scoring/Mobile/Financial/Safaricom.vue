...
const loadMshariData = async () => {
  await axiosInstance
    .get(
      `/e_statement/mshwari_summary?statementRefId=${route.params.slug}&pageSize=100&sortBy=id`
    )
...
const loadKcbMpesaData = async () => {
  await axiosInstance
    .get(
      `/e_statement/kcb_mpesa_summary?statementRefId=${route.params.slug}&pageSize=100&sortBy=id`
    )
...
const loadFulizaData = async () => {
  await axiosInstance
    .get(
      `/e_statement/fuliza_summary?statementRefId=${route.params.slug}&pageSize=100&sortBy=id`
    )
