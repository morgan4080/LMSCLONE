...
const { pageables, fetchTodayCollections, setSelectedExportOption, setSelectedStatusOption } = useToday()
const { headers, isLoading, selectedStatusOption, statusOptions, selectedExportOption, exportOptions } = storeToRefs(useToday())
...
<template>
  <v-row class="d-flex justify-center my-2">
    <h3 class="pa-2 font-weight-regular">
      Due Today({{ pageables.totalRecords }})
    </h3>
    <v-spacer></v-spacer>
